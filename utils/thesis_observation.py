...
Structure:
  A. classify_thesis() pure-function correctness (in-process; the function
     itself has no persistence/process-boundary concerns)
  B. Zero-strength-score semantics (in-process, pure function)
  C. Shared strength-matrix call-count proof (in-process; about NOT
     re-invoking a function, provable within one process)
  D. Missing/corrupt persistence file safety (in-process; deterministic
     file-content manipulation, no need for a second process)
  E. Orphan cleanup (in-process)
  F. Passivity re-proof against the corrected signature (in-process)
  G. REAL SUBPROCESS-BOUNDARY TESTS — the mandatory core of this suite.
     Each spawns an actual `python -c "..."` child process via
     subprocess.run([sys.executable, ...]) per the task's explicit
     instruction not to simulate process boundaries by calling the
     function twice in-process. These are the tests that would have
     caught the original defect; the old in-process-only suite could not.
"""
import json
import os
import sys
...
def test_shared_matrix_computed_once_across_multiple_instruments(isolate):
    ...
    with patch(
        "utils.strategy_helpers.build_strength_matrix",
        side_effect=_counting_matrix
    ), p1, p2, p3:
...
# ---------------------------------------------------------------------------
# D. Missing / corrupt persistence file
# ---------------------------------------------------------------------------
...
_CYCLE_SCRIPT = textwrap.dedent("""
    ...
    with patch(
        "utils.strategy_helpers.check_ma5_alignment",
        return_value={alignment!r}
    ), \\
         patch(
             "utils.strategy_helpers.get_slope_diagnostics",
             return_value={{"combined": "AGAINST"}}
         ), \\
         patch(
             "utils.strategy_helpers.get_atr_with_volatility_context",
