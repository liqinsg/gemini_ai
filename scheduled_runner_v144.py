...
==========================================
v1.4.4 auditable execution enhancement:
...
def _validate_and_repair_sltp(report: dict, dry_run: bool):
    ...
    for trade in open_trades:
        ...
        calculated_sl = format_price_for_instrument(
            entry - SL_PIPS * pip, instrument
        ) if side == "BUY" else format_price_for_instrument(
            entry + SL_PIPS * pip, instrument
        )
        calculated_tp = format_price_for_instrument(
            entry + TP_PIPS * pip * TP_RATIO, instrument
        ) if side == "BUY" else format_price_for_instrument(
            entry - TP_PIPS * pip * TP_RATIO, instrument
        )
...
def run_cycle(dry_run=None):
    ...
    try:
        ...
        for candidate in sorted(candidates, key=lambda value: abs(value.get("strength_score", 0.0)), reverse=True):
            ...
            if gate_enabled:
                try:
                    pair_mc = get_latest_mc_local(pair=pair, day=True)
                    gate = PostExitGate.evaluate(instrument=pair, action=action, strength_score=candidate.get("strength_score", 0.0), candidate_rank=1, mc_regime_raw=(pair_mc or {}).get("regime", "NO_LOCAL_MC_DATA"), baseline_units=profile["units"], tracker=None)
                    if gate_shadow:
            ...
            if execute_market_trade(signal, units_override=effective_units, client_extensions=build_client_extensions(candidate, strategy_tag=strategy_tag, bar_time=candidate.get("bar_time"))):
                ...
                report["reason"] = "Existing strategy qualification rules passed and an entry request was submitted."
                report["next_plan"] = f"Position: {pair} {action}; recalculate SL/TP every cycle and request changes only when the threshold is exceeded."
            else:
