...
def manage_open_positions() -> set:
    """
    Phase A (new): revisit every currently risk-managed instrument —
    reconcile against OANDA, compute this cycle's RiskAction, and execute
    it (SL update / partial close / full close). No-op, returns empty set,
    if the flag is off.

    Returns:
        Set of instruments that were (successfully or attemptedly) managed
        this cycle — used by run_cycle() to avoid re-entering a pair the
        risk layer is already handling.
    """
    ...
    managed = set()
    try:
        instruments = _risk.list_managed_instruments()
    ...
    for instrument in instruments:
        managed.add(instrument)  # reserve this pair regardless of outcome below —
                                  # if we're not sure of its state, don't also try to enter it fresh
        try:
            cluster_data = _risk.load_cluster_data(instrument)
            if cluster_data is None:
                continue  # deleted between list and load (e.g. by a previous iteration) — fine

            cluster = _risk.restore_cluster(cluster_data)

            still_open = _risk.reconcile_with_oanda(cluster, instrument)
            if not still_open:
                _risk.delete_cluster_data(instrument)
                print(f"  [RISK] {instrument} closed externally (TP/manual) — removed from managed state.")
                continue

            price, atr_now, hh, ll = _risk.fetch_market_context(instrument, cluster)
            ...
            _risk.apply_risk_action(cluster, instrument, action)
            ...
            if cluster.risk_manager.state == RiskStateEnum.CLOSED:
                ...
                _risk.save_cluster_data(instrument, cluster.to_dict())
            ...
            # KNOWN GOOD state in place and retry fresh next cycle, rather than
            # risk persisting a half-applied mutation or losing track of a live position.
...
def run_cycle():
    ...
    managed_instruments = manage_open_positions()
